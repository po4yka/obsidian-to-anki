...
def run_nested_test(num_notes, cards_per_note, max_workers):
    start = time.time()
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(process_note_nested, cards_per_note, max_workers) for _ in range(num_notes)]
...
def run_flat_test(num_notes, cards_per_note, max_workers):
    start = time.time()
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(process_note_sequential, cards_per_note) for _ in range(num_notes)]
        results = sum(f.result() for f in futures)
    return time.time() - start
...
if __name__ == "__main__":
    ...
    print(f"Simulating {NUM_NOTES} notes, {CARDS_PER_NOTE} cards/note, max_workers={MAX_WORKERS}")
