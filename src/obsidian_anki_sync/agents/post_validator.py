...
- Tags: space-separated (not comma-separated)
            ...
            try:
                ...
                base_temperature = (
                    self.temperature if self.temperature is not None else 0.0
                )
                ...
                logger.info(
                    "auto_fix_llm_failed_trying_aggressive_deterministic",
                    error=str(llm_error),
                )
    ...
    def _apply_deterministic_fixes(
        ...
        for card in cards:
            ...
            if "Missing manifest" in error_details or (
                "manifest" in error_details.lower()
                and "missing" in error_details.lower()
            ):
            ...
            if "HTML" in error_details and (
                "code" in error_details.lower() or "inline" in error_details.lower()
            ):
            ...
            if "tag" in error_details.lower() and (
                ...
                if tags_match:
                    ...
                    if fixed_tags != tags:
                        ...
                        fixed_html = re.sub(
                            r"(Tags:\s*)([^>]+)", rf"\1{new_tags_str}", fixed_html
                        )
                        any_fixes = True
                        card_fixed = True
                        logger.debug(
            ...
            if (
                "Missing required field header" in error_details
                or "field header" in error_details.lower()
            ):
            ...
            if (
                "Invalid manifest JSON" in error_details
                or "manifest JSON" in error_details.lower()
            ):
