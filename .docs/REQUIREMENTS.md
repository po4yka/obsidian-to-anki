...
- Для каждой карточки вычисляется детерминированный GUID (SHA‑1 → base64, 16 символов) на основе `note_id`, относительного пути заметки, индекса пары и языка. Этот GUID передаётся в Anki при создании/восстановлении и сохраняется в manifest (`guid`) и state DB (`card_guid`).
...
- Выходной HTML каждой карточки валидируется: кодовые блоки обязаны использовать `<pre><code class="language-...">`, запрещены Markdown‑фенсы ``` и одиночные `<code>` вне `<pre>`. Нарушения приводят к ошибке синхронизации.
